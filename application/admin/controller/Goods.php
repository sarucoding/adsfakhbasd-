...
class Goods extends Base {
    ...
    public function addEditCategory()
    {
        ...
        if (IS_GET) {
            ...
            $all_type = Db::name('goods_category')->where("level<2")->getField('id,name,parent_id');//上级分类数据集，限制2级分类，那么只拿前一级作为上级选择 （分类整体修改为二级结构）
            if(!empty($all_type)){
            ...
            $goods_category_list = Db::name('goods_category')->where("id in(select cat_id1 from {$db_prefix}goods_type) ")->getField("id,name,parent_id");
            $goods_category_list[0] = array('id' => 0, 'name' => '默认');
            asort($goods_category_list);
            $this->assign('goods_category_list', $goods_category_list);
            
            $goods_type_list = Db::name('goods_type')->select(); // 所有类型id
    ...
    public function imgList(){
        ...
        $count = Db::name('image_extend')->where($where)->count();
        $pager  = new Page($count,10);
        $imgList = Db::name('image_extend')->where($where)->order('img_id desc')->limit($pager->firstRow.','.$pager->listRows)->select();
    ...
    public function addImg(){
        if(IS_POST){
            $goods_name = I('goods_name/a');
            $fileList = request()->file('img');
            $ossClient = new OssLogic();
            foreach($fileList as $file){
                $filePath = $file->getRealPath();
                $new_name = md5(time()+build_serial_number()).'.'.pathinfo($file->getInfo('name'), PATHINFO_EXTENSION);
                $en_name[] = $new_name;
                $cn_name[] = $file->getInfo('name');
                $object = 'library/'.$new_name;

                $return_url = $ossClient->appendFile($filePath, $object);
                if (!$return_url) {
                    $this->error($ossClient->getError());
                }
                $img_url[] = $return_url;
            }
            $data = [];
            foreach($goods_name as $k=>$val){
                $data[$k]['goods_name'] = $val;
                $data[$k]['en_name'] = $en_name[$k];
                $data[$k]['cn_name'] = $cn_name[$k];
                $data[$k]['img_url'] = $img_url[$k];
            }
            $res = model('image_extend')->saveAll($data);
            if(!$res){
                $this->error('添加失败');
            }
            $this->success('添加成功');
    ...
    public function initLocationJsonJs()
    {
        ...
        $data = M('region')->where(['level'=>['lt',4]])->select();
        $arr = [];
        $level_three = [];
        foreach($data as $key=>$value){
            if($value['level'] == 1){
              $arr[$value['id']]['n'] = $value['name'];
              $arr[$value['id']]['id'] = $value['id'];
            }
            if($value['level'] == 2){
              $arr[$value['parent_id']]['c'][$value['id']]['n'] = $value['name'];
              $arr[$value['parent_id']]['c'][$value['id']]['id'] = $value['id'];
            }
            if($value['level'] == 3){
              $level_three[] = $value;
            }
        ...
        foreach($arr as $k=>$v){
            $arr[$k]['c'] = array_values($v['c']);
            foreach($arr[$k]['c'] as $key=>$val){
              foreach($level_three as $i=>$value){
                if($val['id'] == $value['parent_id']){
                  $arr[$k]['c'][$key]['a'][$value['id']]['n'] = $value['name'];
                  $arr[$k]['c'][$key]['a'][$value['id']]['id'] = $value['id'];
                }
              }
            }
        }
        foreach($arr as $k=>$v){
            foreach($v['c'] as $key=>$val){
              if($val['c']){
                $arr[$k]['c'][$key]['a'] = array_values($val['a']);
              }
            }
        }
        $str = "!function(){var citys=".json_encode($arr, JSON_UNESCAPED_UNICODE).';if(typeof define==="function"){define(citys)}else{window.YDUI_CITYS=citys}}();';
        file_put_contents(ROOT_PATH."public/address/js/ydui.citys.js", $str);
