...
class Uploadify extends Base{
    ...
    public function fileList(){
		...
		if($is_library){
			...
			$count = M('Image_extend')->where($where)->count();
			$Page = new AjaxPage($count, 54);
			$returnList = M('Image_extend')->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->field("img_url as url , goods_name as name")->select();
			...
			foreach ($returnList as $k => $v) {
				...
				$returnList[$k]['name'] = $highlightTextList[0];
				$returnList[$k]['title'] = $v['name'];
			}
			$show = $Page->show();
    	...
    	//$files = $adminLogc->getfiles($path, $allowFiles, '', false);//key值作为图库的查询条件了
    	if (!count($files)) {
    	...
    	$where = implode($urls, ",");
		$where = ['img_url'=>['in',$where]];
		if($key){
			$where = ['goods_name'=>['like',"%".$key."%"]];
		}
    	$extends = M('Image_extend')->where($where)->getField("img_url , cn_name , en_name , goods_name" , true);
    	$returnList = array();
    	foreach ($list as $k =>$v){
			if($key){
				if($extends[$v['url']]){//如果在数据库内找得到对应的数据
					$returnList[$k] = $v;
					$extends[$v['url']]['goods_name'] ? $returnList[$k]['name'] = $extends[$v['url']]['goods_name'] : $returnList[$k]['name'] = $extends[$v['url']]['cn_name'];
				}
			}else{
				$returnList[$k] = $v;
				$extends[$v['url']]['goods_name'] ? $returnList[$k]['name'] = $extends[$v['url']]['goods_name'] : $returnList[$k]['name'] = $extends[$v['url']]['cn_name'];
