...
class Report extends Base{
    ...
    public function finance(){
        ...
        $order = Db::name('order')->alias('o')
            ->where(['o.pay_status'=>2,'o.shipping_status'=>1,'o.order_status'=>['in','1,2,4'],'o.store_id'=>$this->store_id])->whereTime('o.add_time', 'between', [$begin, $end_time])
        ...
        $order_goods = Db::name('order_goods')->where(['is_send'=>['in','1,2'],'order_id'=>['in',$order_ids]])->group('order_id')
            ->order('order_id asc')->getField('order_id,sum(total_cost) as total_cost,sum(goods_num*member_goods_price) as goods_amount');  //订单商品退货的不算
        ...
        for($i=$sratus_date;$i<=$end_date;$i=$i+24*3600){   //循环时间
            ...
            $goods_amount=$cost_price =$shipping_amount=$coupon_amount=$order_prom_amount=$total_amount=$profit_amount=0.00; //初始化变量
            foreach ($order as $okey => $oval){   //循环订单
                ...
                if($oval['add_time'] >= $i && $oval['add_time']<$everyday_end_time){      //统计同一天内的数据
                    ...
                    $cost_price        += $order_goods["$for_order_id"]['total_cost']; //订单成本价
                    $shipping_amount   += $oval['shipping_price'];
                    $coupon_amount     += $oval['coupon_amount'];
                    $profit_amount     += $oval['goods_price'] - $order_goods["$for_order_id"]['total_cost'];
        ...
        rsort($list);
        $this->assign('list',$list);
        $result = ['goods_arr'=>$goods_arr,'cost_arr'=>$cost_arr,'shipping_arr'=>$shipping_arr,'coupon_arr'=>$coupon_arr,'time'=>$day,'profit_arr'=>$profit_arr];
        $this->assign('result',json_encode($result));
        return $this->fetch();
    }
}