...
class OssLogic
{
     ...
     * @return bool|string
     */
    public function appendFile($filePath, $object = null)
    {
        $ossClient = self::getOssClient();
        if (!$ossClient) {
            return false;
        ...
        try {
            $ossClient->appendFile(self::$bucket, $object, $filePath, 0);
        } catch (OssException $e) {
            self::$errorMsg = "oss上传文件失败，" . $e->getMessage();
            return false;
        }
     ...
     * 获取指定目录文件列表
     */
    public function fileList($prefix = '')
    {
        $ossClient = self::getOssClient();
        if (!$ossClient) {
            return false;
        ...
        foreach ($objectList as $val) {
            $fileList[] = $this->getSiteUrl() . '/' . $val->getKey();
        }
        return $fileList;
    }
}