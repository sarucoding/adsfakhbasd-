...
class Goods extends Base
{
    ...
    public function goodsInfo()
    {
        ...
        $goods_attribute = db('goods_attribute')->getField('attr_id,attr_name'); // 查询属性
        ...
        $im_data = array();
        if(session('?user')){
            $im_data = session('user');
            $im_data['head_pic'] = empty($im_data['head_pic']) ?  '' : 'http://'.$_SERVER['HTTP_HOST'].$im_data['head_pic'];
        }
		$this->assign('imData',$im_data);
