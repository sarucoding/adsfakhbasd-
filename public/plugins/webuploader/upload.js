$(function() {
	...
	window.Manager = {
		...
		showSearch : function(key='',page=1){
			...
			$.ajax({
				...
				success : function(data){
					...
					if(typeof $html == 'string') $container.append($html);
						...
						$('#search_area').find('.dataTables_paginate').remove();//先删除之前的分页标签
						for(var i = 0; i < $html.length; i++){
							if(i == $html.length-1){
								$('#search_area').append($html[i]);
							}else{
		...
		createFile : function(data){
			...
			if(data && data.list && data.list.length > 0){
				...
				for(; i < data.list.length; i++){
					...
					file = '<div class="img" title="' + t.title + '"><img width="100%" src="' + t.url + '" /><span class="icon"></span></div>';

					//if(!/png|jpg|jpeg|gif|bmp/.test(ext)) file = '<div class="img file-global file-' + ext + '" title="' + t.name + '"><span class="icon"></span></div>';

					del = data.is_library ? '' : '<div class="file-panel"><span class="cancel">删除</span></div>';//如果是图库的图片则不允许删除
					$li = $('<li class="file">' + del +  file + '<div class="desc"><a style="color: #666;" target="_blank" href="'+t.url+'">' + t.name + '</a></div></li>');
