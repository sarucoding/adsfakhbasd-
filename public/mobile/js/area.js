...
//地区选择
var $areaList = $(".modalbox-selectzone .zone-detail .content");
var selectedLoc;
/*初始化省份*/
function initProvince(){
	var areaProvince = "";
	for (var i = 0; i < province.length; i++) {
		areaProvince += '<li onclick="selectProvince(' + i + ')">' + province[i] + '</li>';
	}
	$areaList.html(areaProvince);
	$(".modalbox-selectzone .title-head .back").removeAttr("onclick").hide();
	$(".modalbox-selectzone .zone-detail").removeClass("anima");
}
initProvince();
//选择省份
function selectProvince(p){
	...
	setTimeout(function(){
		...
		areaCity += '<li onclick="selectCity(' + p + ',' + j + ');">' + city[p][j] + '</li>';
		}
		$areaList.html(areaCity);
		$(".modalbox-selectzone .zone-detail").addClass("anima");
	}, 300);
	
	selectedLoc = province[p];
	$(".modalbox-selectzone .title-head .back").attr("onclick", "initProvince();").show();
}
//选择城市
function selectCity(p, c){
	$(".modalbox-selectzone .zone-detail").removeClass("anima");
	var areaZone = "";
	setTimeout(function(){
		for (var k=0; k<district[p][c].length; k++) {
			areaZone += '<li onclick="selectZone(' + p + ',' + c + ',' + k + ');">' + district[p][c][k] + '</li>';
		}
		$areaList.html(areaZone);
		$(".modalbox-selectzone .zone-detail").addClass("anima");
	}, 300);   
	var sCity = city[p][c];
	if (sCity != "省直辖县级行政单位") {
		if (sCity == "东莞市" || sCity == "中山市" || sCity == "儋州市" || sCity == "嘉峪关市") {
			selectedLoc += sCity;
			$(".addnew-zone input").val(selectedLoc);
			closeAddressWin();
	...
	$(".modalbox-selectzone .title-head .back").attr("onclick", "selectProvince(" + p + ");");
}
//选择县城
function selectZone(p, c, k){
	$(".modalbox-selectzone .zone-detail").removeClass("anima");
	selectedLoc += district[p][c][k];
	$(".addnew-zone input").val(selectedLoc);
	closeAddressWin();
}
//选择完关闭地区选择窗口
...
//点击相关地方给一个颜色反馈
$(".modalbox-selectzone .zone-detail .content").on("click", "li", function(){
	$(this).css({color: "#F35338"});
});